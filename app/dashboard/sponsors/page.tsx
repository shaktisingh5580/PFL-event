...
const DEMO_SPONSORS = [
  { id:1, company:"Infosys", industry:"Technology", tier:"Platinum", email:"sponsor@infosys.com", phone:"+91-9876543210", event_type_id:"techfest" },
  { id:2, company:"TCS", industry:"IT Services", tier:"Gold", email:"events@tcs.com", phone:"+91-9123456789", event_type_id:"techfest" },
  { id:3, company:"Wipro", industry:"Software", tier:"Silver", email:"partner@wipro.com", phone:"", event_type_id:"hackathon" },
  { id:4, company:"Razorpay", industry:"Fintech", tier:"Gold", email:"brand@razorpay.com", phone:"+91-9988776655", event_type_id:"hackathon" },
  { id:5, company:"Zomato", industry:"Food Tech", tier:"Bronze", email:"collab@zomato.com", phone:"+91-9900112233", event_type_id:"cultural" },
  { id:6, company:"HDFC Bank", industry:"Banking", tier:"Platinum", email:"csponsor@hdfcbank.com", phone:"+91-9090807060", event_type_id:"sports" },
];

const TIER_CLASSES: Record<string, string> = { Platinum:"tier-platinum", Gold:"tier-gold", Silver:"tier-silver", Bronze:"tier-bronze" };

export default function SponsorsPage() {
  const [sponsors, setSponsors] = useState(DEMO_SPONSORS);
  ...
  const toggleAll = () => setSelected(prev => prev.length === filtered.length ? [] : filtered.map(s=>s.id));
  ...
  const openPreview = async () => {
    ...
    try {
      const res = await fetch(`${API}/api/sponsors/preview-email`, { method:"POST", headers:{"Content-Type":"application/json"}, body: JSON.stringify({ sponsor_ids: selected }) });
      const data = await res.json();
      setPreviewEmail(data.email || "Subject: Partnership Opportunity for TechFest 2026\n\nDear Partner,\n\nWe are delighted to invite you to sponsor TechFest 2026...\n\nBest regards,\nPFL Events Team");
    } catch { setPreviewEmail("Subject: Partnership Opportunity for TechFest 2026\n\nDear Partner,\n\nWe are delighted to invite you to sponsor TechFest 2026, a premier technology event expecting 500+ participants.\n\nBest regards,\nPFL Events Team"); }
  ...
  const sendEmails = async () => {
    ...
    setSending(true);
    try {
      await fetch(`${API}/api/sponsors/send-emails`, { method:"POST", headers:{"Content-Type":"application/json"}, body: JSON.stringify({ sponsor_ids: selected, from_email: fromEmail, app_password: appPassword }) });
    ...
    <div>
      ...
      <div style={{display:"flex", alignItems:"center", gap:12, marginBottom:20, flexWrap:"wrap"}}>
        <select className="input-glass" style={{width:200}} value={filterType}
          onChange={(e) => { setFilterType(e.target.value); if (e.target.value !== "all") fetchSponsors(e.target.value); }}>
      ...
      <div className="glass" style={{marginBottom:16, overflow:"auto"}}>
        {loading ? (
          ...
          <table className="data-table">
            <thead>
              <tr>
                <th><input type="checkbox" onChange={toggleAll} checked={selected.length === filtered.length && filtered.length > 0} style={{accentColor:"#a855f7"}}/></th>
      ...
      <div className="glass" style={{overflow:"hidden"}}>
        <button className="btn-outline" style={{width:"100%", borderRadius:0, border:"none", borderBottom: callListOpen ? "1px solid rgba(255,255,255,0.06)" : "none", padding:"14px 20px", justifyContent:"space-between"}}
      ...
      {emailModal && (
        <div className="modal-overlay" onClick={() => setEmailModal(false)}>
          <div className="modal-box" onClick={e => e.stopPropagation()}>
            <div style={{fontWeight:700, fontSize:18, marginBottom:16}}>📧 Email Preview</div>
            {previewLoading ? (
